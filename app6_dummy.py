...
if user_input:
    ...
    with st.spinner("Processing your request..."):
        ...
        if missing_fields:
            ...
            Output a points structure itinerary day by day with:
            - activities
            - restaurants
            - weather
            - optional travel info (bus/plane/cab)(but dont give with ** symbols)
            """
            ...
            with st.chat_message("assistant"):
                ...
                try:
                    ...
                    itinerary = data["tripItinerary"]
                    
                    st.subheader(f"Trip to {itinerary['destination']}")
                    st.markdown(f"**Dates:** {itinerary['startDate']} to {itinerary['endDate']}")
                    st.markdown(f"**People:** {itinerary['numPeople']}")
                    st.markdown(f"**Estimated Cost:** ₹{itinerary['estimatedCost']}")
                    
                    for day in itinerary["days"]:
                        with st.expander(f"**Day {itinerary['days'].index(day) + 1} - {day['date']}**"):
                            st.markdown(f"**Weather:** {day['weather']}")
                            ...
                            if day.get("travelInfo"):
                                st.markdown("**Travel Info:**")
                                st.write(day["travelInfo"])
